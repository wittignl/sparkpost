...
describe('Recipient Lists Library', function() {
    ...
    describe('create', function() {
        ...
        it('should throw an error if no recipients are provided', function() {
            ...
            return Promise.all([
                expect(recipientLists.create(), 'no recipient list hash at all').to.be.rejectedWith(
                    'recipient list is required'
                ),
                expect(recipientLists.create({}), 'no recipients key').to.be.rejectedWith('recipient list is required'),
                expect(recipientLists.create(function() {}), 'recipient list is actually a callback').to.be
                    .rejectedWith('recipient list is required')
            ]);
        });
